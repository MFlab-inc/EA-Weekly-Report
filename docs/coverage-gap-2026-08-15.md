...
**2026-08-15追記（解消）**: 本ドキュメント作成時点で未追跡だった7中銀のうちRBA以外6中銀（日銀・FRB・ECB・BOE・BOC・RBNZ）はtask #19〜#21・#25で担当ソースを追加済み。残っていたRBNZ・SNBもしょうさんの一次ソース直接確認により解決し、8/8中銀すべてがpolicy_rate担当ソースを持つ状態になった（`test/expected-coverage.test.js`の回帰ゲートで確認・グリーン）。詳細は`docs/annual-schedule-maintenance.md`のRBNZ/SNB/BOC各節を参照。
