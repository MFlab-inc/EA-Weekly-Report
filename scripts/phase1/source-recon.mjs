...
const SOURCES = [
  {
    id: 'bls', name: '米労働統計局（BLS）', robotsHost: 'https://www.bls.gov',
    targets: [
  ...
  {
    id: 'rba', name: '豪州準備銀行（RBA）', robotsHost: 'https://www.rba.gov.au',
  ...
  {
    id: 'census', name: '米国勢調査局（Census Bureau）', robotsHost: 'https://www.census.gov',
    targets: [
    ...
    annualHint: '政府横断PFEIスケジュール（CPI/PPI/GDP/小売/貿易収支等を含む）が毎年9月頃に翌年分公表。ただしJOLTS非収録の可能性',
  },
  {
    id: 'ons', name: '英国家統計局（ONS）', robotsHost: 'https://www.ons.gov.uk',
  ...
  {
    id: 'statsnz', name: 'ニュージーランド統計局（Stats NZ）', robotsHost: 'https://www.stats.govt.nz',
    targets: [{ label: 'release_calendar', url: 'https://www.stats.govt.nz/release-calendar/' }],
    annualHint: '長期リリース計画ページの存在は確認。事前公表期間（月数）は未確定',
  },
  {
    id: 'statcan', name: 'カナダ統計局（Statistics Canada）', robotsHost: 'https://www150.statcan.gc.ca',
    targets: [
  ...
  {
    id: 'abs', name: '豪州統計局（ABS）', robotsHost: 'https://www.abs.gov.au',
  ...
  {
    id: 'ism', name: 'ISM', robotsHost: 'https://www.ismworld.org',
    targets: [{ label: 'rob_report_calendar', url: 'https://www.ismworld.org/supply-management-news-and-reports/reports/rob-report-calendar/' }],
    annualHint: '製造業=毎月第1営業日、非製造業=第3営業日・米東部時間10:00が既定則（祝日ずれのみカレンダー確認要）。構造化フィードなし',
  },
  {
    id: 'boj', name: '日本銀行（BOJ）', robotsHost: 'https://www.boj.or.jp',
    targets: [{ label: 'mpm_2026_schedule_pdf', url: 'https://www.boj.or.jp/en/mopo/mpmsche_minu/m_ref/mref250731a.pdf' }],
...
(async () => {
  ...
  for (const src of SOURCES) {
    ...
    const entry = { id: src.id, name: src.name, annualHint: src.annualHint, robots: null, targets: [] };

    // 1) robots.txt
    const robotsUrl = `${src.robotsHost}/robots.txt`;
    const robotsRes = await fetchOne(robotsUrl);
    await sleep(WAIT_MS);
    if (robotsRes?.ok) {
      const text = robotsRes.buf.toString('utf8');
      const groups = parseRobots(text);
      entry.robots = { status: robotsRes.status, bytes: robotsRes.bytes, groupCount: groups.length, raw: text.slice(0, 3000) };
      log(`[robots] ${robotsUrl} -> HTTP ${robotsRes.status}, ${groups.length} user-agent group(s)`);
      log(text.slice(0, 1500));
      writeFileSync(join(OUT_DIR, `${src.id}.robots.txt`), text);

      // 2) 各対象パスがrobots許可かを判定してから取得
      for (const t of src.targets) {
        const u = new URL(t.url);
        const verdict = isDisallowed(groups, u.pathname);
        if (verdict.disallowed) {
          log(`[SKIP-DISALLOWED] ${t.label}: ${t.url} — robots.txt Disallow: ${verdict.rule}`);
          entry.targets.push({ label: t.label, url: t.url, fetched: false, reason: `robots disallow: ${verdict.rule}` });
          continue;
        }
        const res = await fetchOne(t.url, `${src.robotsHost}/`);
        await sleep(WAIT_MS);
        if (res?.ok) {
          ...
          entry.targets.push({
            ...
            contentType: res.contentType, sha256: res.sha256, savedAs: filePath,
          });
        } else {
      ...
      log(`[robots-ERR] ${robotsUrl}: ${robotsRes?.error || robotsRes?.status}. robots.txt不明のため全対象パスをSKIPし手動確認が必要と記録`);
      entry.robots = { error: robotsRes?.error || `HTTP ${robotsRes?.status}` };
      for (const t of src.targets) {
        entry.targets.push({ label: t.label, url: t.url, fetched: false, reason: 'robots.txt取得失敗のため未実施（要手動確認）' });
      }
    }
  ...
  for (const s of report.sources) {
    ...
    log(`${s.id}: robots=${s.robots?.error ? 'ERROR' : 'OK'} fetched=${okCount}/${s.targets.length}`);
  }
