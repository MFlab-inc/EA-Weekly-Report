...
// task #50/51（2026-08-15、しょうさんのManus突合指摘）の回帰テスト: 同一PDFに既に含まれていた
// Retail tradeがkind未登録のため取りこぼされていた（8/17週のCA小売売上高=8/21発表分がその欠損事例）
test('extractStatCanSchedule: 小売売上高（Retail trade）も同一PDFから抽出できる（8/17週の欠損事例の回帰テスト）', async () => {
  const buf = readFileSync(fixturePath);
  const { text } = await pdf(buf);
  const schedule = extractStatCanSchedule(text);

  assert.ok(schedule.retail_sales.some((e) => e.releaseDate === '2026-08-21' && e.referencePeriod === '2026-06'), 'CA小売売上高(8/21=6月分)が見つからない');
  const months2026 = schedule.retail_sales.filter((e) => e.releaseDate.startsWith('2026')).map((e) => e.referencePeriod);
  assert.ok(months2026.length >= 10, `retail_sales: 2026年分の件数が少なすぎる（${months2026.length}件）`);
});
